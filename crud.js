...
add.onclick = await function(){
    ...
    if(dateTime == "" || notes == "" ){
        ...
        if(dateTask >= dateNow){
            ...
            const newId = fire.doc(fire.collection(db,"teste coleção"))
            fire.setDoc(newId,{dateTime,notes})

            //const docId = fire.addDoc(fire.collection(db,"coleção"),{dateTime,notes})
            //fire.doc(fire.collection(db,"teste"))
